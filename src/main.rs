...
fn actual_main() -> anyhow::Result<()> {
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async move {
            ...
            error!("Secondary loops finished");
        });
    Ok(())
}
...
async fn websocket_loop() -> Result<()> {
    ...
    loop {
        ...
        let (mut ws, _) = async_tungstenite::tokio::connect_async(app_config.websocket_url)
            .await
            .map_err(|e| anyhow::anyhow!("Error connecting to websocket: {:?}", e))?;
